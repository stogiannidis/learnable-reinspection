...
`poc/` contains the synthetic grid proof-of-concept: data generation, baseline vs. re-inspection models, training, evaluation, and figure code. **`reinspection_vlm/`** is the unified multi-backend training stack supporting Qwen3-VL, Qwen2.5-VL, InternVL3, and Gemma 4 (shared module, training, eval, `backends/` wrappers, and **`reinspection_vlm/scripts/`** launch scripts). **Hydra** composes defaults from `reinspection_vlm/configs/config.yaml` plus groups `configs/backend/` (`qwen3vl`, `qwen25vl`, `internvl3`, `gemma4`) and `configs/stage/`; DeepSpeed JSON lives under `reinspection_vlm/configs/`. Older InternVL checkpoints: use Hydra `backend=internvl3_legacy` (extends `internvl3` with smaller bottleneck/query settings). Standalone reference YAML for the same hyperparameters is kept under `reinspection_vlm/config_archive/internvl3/` (`stage1_legacy.yaml`, `stage2_legacy.yaml`). `k8s/` holds Kubernetes job manifests, `logs/` stores run logs, **`models/`** is the default root for saved checkpoints (`{output_dir}/{backend}/stage{N}/epoch_{E}/`), and `doc.md` / `simpledoc.md` explain the method and expected outputs.
...
- `pip install -r reinspection_vlm/requirements.txt`: install the unified VLM training stack (Qwen3-VL, Qwen2.5-VL, InternVL3, Gemma 4).
- `bash reinspection_vlm/scripts/run_stage1.sh` / `run_stage2.sh` / `run_eval.sh`: training and eval (Hydra defaults use `backend=internvl3`; pass e.g. `backend=qwen3vl`, `backend=qwen25vl`, or `backend=gemma4` as extra args). Example: `bash run_stage1.sh backend=gemma4 data_root=/data/datasets wandb_run_name=my-run`.
